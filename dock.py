...
class _PlayerShortcutFilter(QObject):
    ...
    def _should_handle(self) -> bool:
        if not self._dock_alive():
            return False
        if not self._dock.isVisible() or not self._dock._current_item():
            return False
...
class AnkiTubeDock(QDockWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._fullscreen_button.clicked.connect(self._open_fullscreen)
    ...
    def on_budget_changed(self) -> None:
        self._update_budget_ui()
        if self._budget.has_time() and self._paused_for_budget:
            self._paused_for_budget = False
            self._play_current()
        elif not self._budget.has_time() and self._is_playing:
    ...
    def _open_fullscreen(self) -> None:
        ...
        if item:
            start = self._start_seconds_for(item.video_id)
            self._fullscreen._web.eval(
                f"window.ankittube.loadVideo({json.dumps(item.video_id)}, {start});"
            )
            if self._is_playing and self._budget.has_time():
                self._fullscreen._web.eval("window.ankittube.play();")
        self._fullscreen.showFullScreen()
    ...
    def _handle_player_message(self, message: str) -> None:
        ...
        if message == "ended":
            ...
            item = self._current_item()
            if item:
                self._positions_cache.pop(item.video_id, None)
            self._refresh_queue_ui()
            if self._budget.has_time():
                self._play_next()
            else:
                self._handle_budget_exhausted()
            return
